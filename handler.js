    ...
    try {
        ...
        try {
            const defaultUser = {
                ...
                useDocument: false, bank: 0, level: 0, role: 'مبتدئ', premium: false, premiumTime: 0
            };
        ...
        // --- نظام حظر الدردشة / إلغاء الحظر مدمج ---
        if (m.isGroup) {
            ...
            if (['banchat', 'mutechat'].includes(cmd)) {
                ...
                if (chatData.isBanned) return this.reply(m.chat, 'هذه الدردشة محظورة بالفعل.', m);
                chatData.isBanned = true;
                return this.reply(m.chat, '✅ تم حظر الدردشة بنجاح. البوت لن يرد هنا.', m);
            }
            if (['unbanchat', 'unmutechat'].includes(cmd)) {
                if (!isOwner && !isROwner) return dfail('owner', m, this);
                if (!chatData.isBanned) return this.reply(m.chat, 'هذه الدردشة غير محظورة.', m);
                chatData.isBanned = false;
                return this.reply(m.chat, '✅ تم إلغاء حظر الدردشة بنجاح. البوت أصبح نشطًا مرة أخرى.', m);
            }
        }
            ...
            if ((usedPrefix = (match[0] || '')[0])) {
                ...
                if (chatData || _user) {
                    ...
                    if (m.text && _user?.banned && !isROwner) {
                        m.reply(`《✦》أنت محظور/ة، لا يمكنك استخدام الأوامر في هذا البوت!\n\n${_user.bannedReason ? `✰ *السبب:* ${_user.bannedReason}` : '✰ *السبب:* غير محدد'}\n\n> ✧ إذا كان هذا البوت حسابًا رسميًا ولديك دليل يؤكد أن هذه الرسالة خطأ، يمكنك تقديم قضيتك إلى مشرف.`);
                        _user.antispam++;
                        return;
                    }

                    if (_user?.antispam2 && isROwner) return;
                    if (new Date() - (_user?.spam || 0) < 3000) return console.log(`[ سبام ]`);
                ...
                if (xp > 200) m.reply('إجراء غير صالح -_-'); // تم التعريب
                else m.exp += xp;

                // ❌ تم إيقاف نظام التحقق من العملات لجعله مجانياً بالكامل
                /*
                if (!isPrems && plugin.coin && _user.coin < plugin.coin) {
                    conn.reply(m.chat, `❮✦❯ لقد نفدت عملاتك`, m);
                    continue;
                }
                */

                // إصلاح مشكلة التنسيق في رسالة المستوى
                if (plugin.level > _user.level) {
                    conn.reply(m.chat, `❮✦❯ يلزم الوصول إلى المستوى: *${plugin.level}*\n\n• مستواك الحالي: *${_user.level}*\n\n• استخدم الأمر التالي للترقية:\n*${usedPrefix}levelup*`, m);
                    continue;
                }
                ...
                try {
                    ...
                    // ❌ تم إيقاف إضافة أو تحديث خصم العملة من الأمر
                    ...
                    for (let key of Object.values(global.APIKeys || {})) {
                        errText = errText.replace(new RegExp(key, 'g'), 'المشرف');
                    }
                    m.reply(errText);
                } finally {
                    ...
                    // ❌ تم إيقاف رسالة إشعار استهلاك العملات
                    // if (m.coin) conn.reply(m.chat, `❮✦❯ استخدمت ${+m.coin} عملة`, m);
                }
                break;
            }
        }
        ...
        if (m) { 
            let currentUser = global.db.data.users[sender]; // تم تعديل الاسم من utente ليكون واضحاً
            if (currentUser?.muto) {
                await conn.sendMessage(m.chat, { delete: { remoteJid: m.chat, fromMe: false, id: m.key.id, participant: m.key.participant }});
            }
            if (sender && currentUser) {
                currentUser.exp += m.exp;
                // ❌ تم إيقاف خصم العملات من قاعدة البيانات
                // if (m.coin) currentUser.coin -= m.coin;
            }
...
watchFile(file, async () => {
    unwatchFile(file);
    console.log(chalk.green('جاري تحديث "handler.js"'));
